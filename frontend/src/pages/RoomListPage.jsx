...
import { Table, Button, Typography, Space, Tag, Modal, Form, Input, Select } from 'antd';
...
const renderStatusTag = (status) => {
    ...
    const style = statusMap[status] || { color: 'default', text: status.toUpperCase() };
    return <Tag color={style.color}>{style.text}</Tag>;
};

// Component con để hiển thị nội dung
const RoomListContent = () => {
    // SỬA Ở ĐÂY: Lấy đúng các hàm từ context
    const {
        rooms,
        loading,
        handleDelete,
            ...
            render: (_, record) => (
                <Space size="middle">
                    {record.status === 'occupied' ? (
                        <Button danger onClick={() => handleCheckout(record.bookingId)} disabled={!record.bookingId}>
                            Trả phòng
                        </Button>
                    ) : (
            ...
            >
                <Form form={form} layout="vertical">
                    ...
                    <Form.Item name="floor" label="Tầng" rules={[{ required: true }]}>
                     ...
                     <Form.Item name="room_type_id" label="Loại phòng" rules={[{ required: true }]}>
